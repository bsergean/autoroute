...
def run(input_stream, options):
    ...
    #  is non ascii and jenkins hates it
    delim = '∎'
    delim = '*'
    print("# each %s represents a count of %d. total %d" % (delim, scale, total))
    ...
    for value, key in data:
        ...
        print(
            str_format % (key[:max_length], value, (value // scale) * delim, percentage)
        )
...
if __name__ == "__main__":
    ...
    input_stream = load_stream(sys.stdin)

    # print(load_stream(sys.stdin))
    # print(options)

    run(input_stream, options)
